...
def make_flow_timesteps(t_start: int, num_flow_steps: int, verbose=True, shift_scale = 7, mode='normal'):
    '''Select `num_meta_steps` timesteps for meta sampling from 0 down to 1'''
...
class RFSampler(BaseDiffusionSampler):
    ...
    def __init__(self, schedule_shift=False, hunyuan_schedule=False, shift_scale=7, mode='normal', distill=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def prepare_sampling_loop(self, x, cond, uc=None, num_steps=None):
        ...
        if self.hunyuan_schedule:
            sigmas = make_flow_timesteps(0, self.num_steps, mode=self.mode, shift_scale=self.shift_scale)
    ...
    def denoise(self, x, denoiser, sigma, cond, uc, scale=None, fps=None):
        additional_model_inputs = {}
        additional_model_inputs['cfg_scale'] = scale if scale is not None else self.guider.scale
        if self.distill:
            denoised = denoiser(x, sigma, cond, **additional_model_inputs).to(torch.float32)
        else:
...
class RFSamplerLong(BaseDiffusionSampler):
    def __init__(self, schedule_shift=False, hunyuan_schedule=False, shift_scale=7, mode='normal', distill=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.schedule_shift = schedule_shift
        self.hunyuan_schedule = hunyuan_schedule
    ...
    def sampler_step(self, sigma, next_sigma, denoiser, x, cond, uc=None, scale=None, fps=None, tile_indices=None, smpl_tiled=None):
        ...
        weight = torch.minimum(weight, 2.0 - weight)    # torch.size(17)

        for k in range(len(tile_indices)-1):
            current_idx = tile_indices[k]
            next_idx = tile_indices[k+1]
            x_current = x[:, current_idx]
            x_next = x[:, next_idx]

            # Get the corresponding condition
            smpl_tile_current = smpl_tiled[:, k]
            smpl_tile_next = smpl_tiled[:, k+1]
            cond_current = cond.copy()
            uc_current = uc.copy()
            cond_next = cond.copy()
            uc_next = uc.copy()

            cond_current["concat_smpl_render"] = smpl_tile_current
            cond_next["concat_smpl_render"] = smpl_tile_next
            uc_current["concat_smpl_render"] = smpl_tile_current
            uc_next["concat_smpl_render"] = smpl_tile_next

            denoised_current = self.denoise(x_current, denoiser, sigma, cond_current, uc_current, scale=scale, fps=fps).to(torch.float32)
            denoised_next = self.denoise(x_next, denoiser, sigma, cond_next, uc_next, scale=scale, fps=fps).to(torch.float32)
        ...
        # output = self.denoise(x, denoiser, sigma, cond, uc, scale=scale, fps=fps).to(torch.float32)    # 在预测噪声时使用CFG
        output = denoised
        # 去掉denoised
        x = x + append_dims(next_sigma - sigma, x.ndim) * output    # 去掉噪音
        return x

    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, scale=None, ofs=None, fps=None, tile_indices=None):
        x, s_in, sigmas, num_sigmas, cond, uc, timesteps = self.prepare_sampling_loop(
            x, cond, uc, num_steps
        )
        smpl_tiled = cond['smpl_tiled']
        for i in self.get_sigma_gen(num_sigmas):
            x = self.sampler_step(
                s_in * sigmas[i],
                s_in * sigmas[i + 1],
