...
class ImagePatchEmbeddingMixin(BaseMixin):
    def __init__(
        ...
        if use_conv:
            ...
            self.proj_mask = nn.Conv3d(28, hidden_size, kernel_size=patch_size, stride=patch_size, bias=bias)
        else:
    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        ...
        ref_mask = kwargs["concat_latent_ref_mask"] # (b,t+1,c,h,w)
        sam_mask = kwargs["concat_latent_sam"] # (b,t,c,h,w)
        if self.use_conv:
            ...
            ref_mask_emb = rearrange(ref_mask, 'b t c h w -> b c t h w')
            emb = self.proj(emb) + self.proj_mask(ref_mask_emb) # (b,c,t+1,h/2,w/2)

            pose_emb = rearrange(pose_emb, 'b t c h w -> b c t h w')
            sam_emb = rearrange(sam_mask, 'b t c h w -> b c t h w')
            pose_emb = self.proj_pose(pose_emb) + self.proj_mask(sam_emb) # (b,c,t,h/4,w/4)
...
class Rotary3DPositionEmbeddingMixin(BaseMixin):
    def __init__(
        ...
        grid_t = torch.arange(0, compressed_num_frames + 1, dtype=torch.float32)  # 修改：从0开始，多一个位置，normal用[1:T+1]，mocha用[0:T]
        grid_h = torch.arange(height, dtype=torch.float32)       # 多加一点，防止越界
        grid_w = torch.arange(width + 120, dtype=torch.float32)

        grid_extended_t = torch.arange(0, 1, dtype=torch.float32)  # ref只用T=0
        grid_extended_h = torch.arange(height + 120, dtype=torch.float32)  # 加H余量，防止ref_rope_H偏移越界
        grid_extended_w = torch.arange(width + 120, dtype=torch.float32)
    ...
    def rotary(self, t, **kwargs):
        if self.pnp:
            ...
            if ref_mask_flag is not None and not ref_mask_flag.all():
                # per-sample: normal T从1开始，mocha T从0开始
                freqs_cos_list = []
                freqs_sin_list = []
                for i in range(t.shape[0]):
                    t_start = 1 if ref_mask_flag[i] else 0
                    def reshape_freq_i(freqs, t_s=t_start):
                        freqs = freqs[t_s:kwargs['rope_T']+t_s, kwargs['rope_H_shift']:kwargs['rope_H']+kwargs['rope_H_shift'], kwargs['rope_W_shift']:kwargs['rope_W']+kwargs['rope_W_shift']].contiguous()
                        freqs = rearrange(freqs, 't h w d -> (t h w) d')
                        return freqs.unsqueeze(0).unsqueeze(0)
                    freqs_cos_list.append(reshape_freq_i(self.freqs_cos))
                    freqs_sin_list.append(reshape_freq_i(self.freqs_sin))
                freqs_cos = torch.cat(freqs_cos_list, dim=0).to(t.dtype)
                freqs_sin = torch.cat(freqs_sin_list, dim=0).to(t.dtype)
            else:
                # 全部normal: T从1开始（与原来行为一致）
                def reshape_freq(freqs):
                    freqs = freqs[1:kwargs['rope_T']+1, kwargs['rope_H_shift']:kwargs['rope_H']+kwargs['rope_H_shift'], kwargs['rope_W_shift']:kwargs['rope_W']+kwargs['rope_W_shift']].contiguous()
                    freqs = rearrange(freqs, 't h w d -> (t h w) d')
    ...
    def rotary_ref(self, t, **kwargs):
        if self.pnp:
            ...
            ref_mask_flag = kwargs.get('ref_mask_flag', None)
            # 检查是否需要per-sample不同的offset
            if ref_mask_flag is not None and not ref_mask_flag.all():
                # per-sample计算freqs，ref_mask_flag为False(Mocha)时H W各偏移
                freqs_cos_list = []
                freqs_sin_list = []
                for i in range(t.shape[0]):
                    if ref_mask_flag[i]:
                        # normal: T=0
                        t_idx = 0
                        h_offset = kwargs['rope_H_shift']
                        w_offset = kwargs['rope_W_shift']
                    else:
                        # mocha: T不偏离，H偏移ref_rope_H
                        t_idx = 0
                        h_offset = kwargs['rope_H_shift'] + kwargs['ref_rope_H']
                        w_offset = kwargs['rope_W_shift']
                    cos_i = self.freqs_extended_cos[t_idx:t_idx+1, h_offset: kwargs['rope_H']+h_offset, w_offset: kwargs['rope_W']+w_offset].contiguous()
                    cos_i = rearrange(cos_i, 't h w d -> (t h w) d').unsqueeze(0).unsqueeze(0)
                    sin_i = self.freqs_extended_sin[t_idx:t_idx+1, h_offset: kwargs['rope_H']+h_offset, w_offset: kwargs['rope_W']+w_offset].contiguous()
                    sin_i = rearrange(sin_i, 't h w d -> (t h w) d').unsqueeze(0).unsqueeze(0)
                    freqs_cos_list.append(cos_i)
                    freqs_sin_list.append(sin_i)
                freqs_cos = torch.cat(freqs_cos_list, dim=0).to(t.dtype)  # (B, 1, seq, d)
                freqs_sin = torch.cat(freqs_sin_list, dim=0).to(t.dtype)
            else:
        ...
        if self.pnp:
            ...
            if ref_mask_flag is not None and not ref_mask_flag.all():
                ...
                for i in range(t.shape[0]):
                    t_start = 1 if ref_mask_flag[i] else 0
                    def reshape_freq_i(freqs, t_s=t_start):
                        freqs = freqs[t_s:kwargs['rope_T']+t_s, kwargs['global_rope_H']+kwargs['rope_H_shift']:kwargs['global_rope_H']+kwargs['rope_H']+kwargs['rope_H_shift'], kwargs['global_rope_W']+kwargs['rope_W_shift']:kwargs['global_rope_W']+kwargs['rope_W']+kwargs['rope_W_shift']].contiguous()
                        freqs = F.avg_pool2d(freqs.permute(0, 3, 1, 2), kernel_size=2, stride=2).permute(0, 2, 3, 1)
                        freqs = rearrange(freqs, 't h w d -> (t h w) d')
                        return freqs.unsqueeze(0).unsqueeze(0)
                    freqs_cos_list.append(reshape_freq_i(self.freqs_cos))
                    freqs_sin_list.append(reshape_freq_i(self.freqs_sin))
                ...
                # 全部normal: T从1开始（与原来行为一致）
                def reshape_freq(freqs):
                    freqs = freqs[1:kwargs['rope_T']+1, kwargs['global_rope_H']+kwargs['rope_H_shift']:kwargs['global_rope_H']+kwargs['rope_H']+kwargs['rope_H_shift'], kwargs['global_rope_W']+kwargs['rope_W_shift']:kwargs['global_rope_W']+kwargs['rope_W']+kwargs['rope_W_shift']].contiguous()
                    freqs = F.avg_pool2d(freqs.permute(0, 3, 1, 2), kernel_size=2, stride=2).permute(0, 2, 3, 1)
                    freqs = rearrange(freqs, 't h w d -> (t h w) d')
                    freqs = freqs.unsqueeze(0).unsqueeze(0)
                    return freqs
...
class DiffusionTransformer(BaseModel):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        if kwargs.get("concat_images", None) is not None: # btchw
            ...
            if kwargs.get("concat_latent_ref_mask", None) is not None:
                if kwargs["concat_latent_ref_mask"].shape[0] != x.shape[0]:
                    concat_latent_ref_mask = kwargs["concat_latent_ref_mask"].repeat(2, 1, 1, 1, 1)
                else:
                    concat_latent_ref_mask = kwargs["concat_latent_ref_mask"]
                kwargs["concat_latent_ref_mask"] = concat_latent_ref_mask

            if kwargs.get("concat_latent_sam", None) is not None:
                if kwargs["concat_latent_sam"].shape[0] != x.shape[0]:
                    concat_latent_sam = kwargs["concat_latent_sam"].repeat(2, 1, 1, 1, 1)
                else:
                    concat_latent_sam = kwargs["concat_latent_sam"]
                kwargs["concat_latent_sam"] = concat_latent_sam
            ...
            if kwargs.get("ref_mask_flag", None) is not None:
                if kwargs["ref_mask_flag"].shape[0] != x.shape[0]:
                    kwargs["ref_mask_flag"] = kwargs["ref_mask_flag"].repeat(2)
